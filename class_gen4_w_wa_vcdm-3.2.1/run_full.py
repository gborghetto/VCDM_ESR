...
def create_cobaya_info_dict(class_path, w_function_index, output_base_path, 
    ...
    output_path = os.path.join(output_base_path, f"{w_function_index:03d}")
    ...
    info = {
        ...
        "params": {
            ...
            "chi2_total": {
                ...
                "derived": "lambda chi2__CMB, chi2__BAO, chi2__SN: chi2__CMB + chi2__BAO + chi2__SN"
            },
        },
        ...
        if num_params == 0:
            ...
            for c_param_name in param_info['param_names']:
                if c_param_name in param_name_map:
                    ...
                    # Set priors based on parameter
                    if cobaya_param_name == 'c0_esr':
                        prior_min, prior_max = -4.0, 4.0
                        ref_loc = 0.01
                        ref_scale = 0.5
                        latex_name = "c0_esr"
...
def run_single_function(class_path, w_function_index, output_base_path,
    ...
    """
    Run minimization for a single w(a) function.
    """
    ...
    min_file = os.path.join(output_path, 'minimize.minimum.txt')
    ...
    # Run minimization
    try:
        print(f"Starting minimization for function {w_function_index}...")
        ...
        print(f"✗ Error during minimization for function {w_function_index}:")
        print(f"  {e}")
        # Save error log
...
def main():
    ...
    output_base = "/scratch/s.2362709/VCDM/class_gen4_w_wa_vcdm-3.2.1/chains" #"/Users/giuliaborghetto/OneDriveSwanseaUniversity/VCDM/chains"
    ...
    # Save summary file
    summary_file = os.path.join(output_base, 'run_summary.txt')
    os.makedirs(output_base, exist_ok=True)
    with open(summary_file, 'w') as f:
        f.write(f"VCDM Multi-Function Analysis Summary\n")
        f.write(f"{'='*70}\n\n")
        f.write(f"Total functions: {num_functions}\n")
        f.write(f"Processed: {len(list(function_indices))}\n")
