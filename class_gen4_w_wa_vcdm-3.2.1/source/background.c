...
// ADF: gen4 code, w w_a code

#include "background.h"
#include "generated_functions.h"

double integrate_w_function_trapezoidal(double a_start, double a_end, int n_steps, 
                                        int func_index, double c0, double c1);
...
int background_functions(
  ...
  /* Lambda */
    // ADF: here modified
  // double w_0_VCDM = pba->w_0_VCDM; // ADF: value of w_0 // AM: can declare spline parameters here
  // double w_a_VCDM = pba->w_a_VCDM; // ADF: value of w_a // AM: can declare spline parameters here
  // int N_nodes_VCDM = pba->N_nodes_VCDM; // AM: number of nodes
  // GB start
  int w_function_index = pba->w_function_index;  // Index into function table
  double c0_esr = pba->c0_esr;   // First parameter for function
  double c1_esr = pba->c1_esr;   // Second parameter for function
  // TEMPORARY DEBUG
  // fprintf(stderr, "DEBUG: w_function_index=%d, c0_esr=%.6f, c1_esr=%.6f\n", 
  //       w_function_index, c0_esr, c1_esr);
  // GB end
  // double * a_nodes_VCDM = pba->a_nodes_VCDM; // AM: array of nodes
  // double * w_nodes_VCDM = pba->w_nodes_VCDM; // AM: array of w values at nodes
  double rho0_VCDM = pba->Omega0_lambda * pow(pba->H0,2); // AM: remains same
  // ADF: next is rho_VCDM, or varrho_VCDM, i.e. in CLASS
  if (pba->has_lambda == _TRUE_) {
    // ADF: adding rhoX(a)
    // pvecback[pba->index_bg_rho_lambda] = rho0_VCDM * pow(a, -0.3e1 - 0.3e1 * w_0_VCDM - 0.3e1 * w_a_VCDM) * exp(0.3e1 * w_a_VCDM * (a - 0.1e1)); 
    // AM: In case of spline, we must modify this to calculate rho_VCDM for given a
    // printf("computing rho_VCDM at a = %e \n", a);
    // pvecback[pba->index_bg_rho_lambda] = rho0_VCDM * exp(integrate_w_spline_trapezoidal(1.0, a, 300, pba->a_nodes_VCDM, pba->w_nodes_VCDM, N_nodes_VCDM));
    // GB start
    pvecback[pba->index_bg_rho_lambda] = rho0_VCDM * exp(integrate_w_function_trapezoidal(1.0, a, 300, w_function_index, c0_esr, c1_esr));
    // Add this temporarily for verification:
    // double analytical_rho = rho0_VCDM * pow(a, -3*(1 + c0_esr + c1_esr)) * exp(3*c1_esr*(a-1));
    // double numerical_rho = rho0_VCDM * exp(integrate_w_function_trapezoidal(1.0, a, 300, w_function_index, c0_esr, c1_esr));
    // if (fabs(a - 0.5) < 0.001) {  // Check at a=0.5
    //     fprintf(stderr, "a=%.3f: analytical=%.6e, numerical=%.6e, ratio=%.6f\n",
    //             a, analytical_rho, numerical_rho, numerical_rho/analytical_rho);
    // }
    // GB end
  ...
  pvecback[pba->index_bg_rho_plus_P_VCDM] = rho_tot + p_tot; // AM: remains same for spline

  // ADF: only now adding rhoX and pX, pX is found by taking the derivative of the Friedmann equation, and by indentifying the X-term as -3/2*a*(rhoX + pX)
  rho_tot += pvecback[pba->index_bg_rho_lambda]; // AM: remains same for spline
  //double rhoX_VCDM_a = -0.2e1 * b_VCDM * (b_VCDM * b2_VCDM * (a - a_star_VCDM) * exp(-b_VCDM * b_VCDM * pow(a_star_VCDM, -0.2e1) * pow(a - a_star_VCDM, 0.2e1)) - a_star_VCDM * pow(0.1e1 / cosh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM), 0.2e1) * (0.1e1 + alpha_VCDM) / 0.4e1) * rho0_VCDM * pow(a_star_VCDM, -0.2e1);
  //double rhoX_VCDM_aa = 0.4e1 * b_VCDM * b_VCDM * (b2_VCDM * ((b_VCDM * b_VCDM - 0.1e1 / 0.2e1) * a_star_VCDM * a_star_VCDM - 0.2e1 * b_VCDM * b_VCDM * a * a_star_VCDM + a * a * b_VCDM * b_VCDM) * exp(-b_VCDM * b_VCDM * pow(a_star_VCDM, -0.2e1) * pow(a - a_star_VCDM, 0.2e1)) + tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM) * a_star_VCDM * a_star_VCDM * (tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM) - 0.1e1) * (0.1e1 + tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM)) * (0.1e1 + alpha_VCDM) / 0.4e1) * rho0_VCDM * pow(a_star_VCDM, -0.4e1);
  //double pX_VCDM = (w_0_VCDM+w_a_VCDM*(1.0 - a))*pvecback[pba->index_bg_rho_lambda]; 
  
  // AM: for spline replaced with w(a)
  // double w_VCDM = w_VCDM_spline(a, pba->a_nodes_VCDM,  pba->w_nodes_VCDM, N_nodes_VCDM, 0); // w_VCDM at given a
  // GB start
  double w_VCDM = function_table[w_function_index](a, c0_esr, c1_esr);
  // pvecback[pba->index_bg_w_fld] = w_VCDM;
  // GB end
  double pX_VCDM = w_VCDM * pvecback[pba->index_bg_rho_lambda];  //w_VCDM_spline(a, a_nodes_VCDM, w_nodes_VCDM, N_nodes_VCDM, 0) 

  p_tot += pX_VCDM; // AM: remains same for spline
  // ADF: Next is adding a*(d p_X / da), where pX is given above
  //dp_dloga += (-0.3e1 * pow(a - 0.1e1, 0.2e1) * w_a_VCDM * w_a_VCDM + (0.6e1 * a * w_0_VCDM + 0.2e1 * a - 0.6e1 * w_0_VCDM - 0.3e1) * w_a_VCDM - 0.3e1 * w_0_VCDM * w_0_VCDM - 0.3e1 * w_0_VCDM)*pvecback[pba->index_bg_rho_lambda];
  // AM: replace with spline derivative, i.e. dpX/da = dw/da * rhoX + w * drhoX/da
  // dp_dloga+= pvecback[pba->index_bg_rho_lambda]* a * (w_VCDM_spline(a, pba->a_nodes_VCDM, pba->w_nodes_VCDM, N_nodes_VCDM, 1) + w_VCDM * (-3*(1 + w_VCDM)/a) ); 
  // GB start
  double dw_da = derivative_table[w_function_index](a, c0_esr, c1_esr);
  dp_dloga += pvecback[pba->index_bg_rho_lambda] * a * (dw_da + w_VCDM * (-3*(1 + w_VCDM)/a));

  // GB end
  // AM : this calculates dp/dloga = a * dp/da; dp/da = dw/da * rhoX + w * drhoX/da; drhoX/da = -3(1+w)/a * rhoX
  // dp/dloga = a * rhoX * ( dw/da + w * (-3(1+w)/a) )
...
int background_free_input(
  ...
  if (pba->Omega0_ncdm_tot != 0.) {
    ...
    // AM added for VCDM spline
    // GB commented a_nodes and w_nodes out
    // if (pba->a_nodes_VCDM != NULL) {
    //   free(pba->a_nodes_VCDM);
    //   pba->a_nodes_VCDM = NULL;
    // }
    // if (pba->w_nodes_VCDM != NULL) {
    //   free(pba->w_nodes_VCDM);
    //   pba->w_nodes_VCDM = NULL;
    // }
    // if (pba->has_lambda == _TRUE_) {
    // free(pba->w_nodes_VCDM);
    // free(pba->a_nodes_VCDM);
...
int background_checks(
  ...
  // AM: added check to ensure that the a-array is sorted in descending order.
  if (pba->has_lambda == _TRUE_) {
    int i;
    // Note: Since w_nodes_VCDM and a_nodes_VCDM are dynamically allocated arrays (pointers),
    // we cannot use sizeof() to get their length. We rely on N_nodes_VCDM for the array size.
    // The arrays are assumed to have been allocated with the correct size during input parsing which CLASS automatically handles.
    // GB start
    // for (i=0; i<pba->N_nodes_VCDM-1; i++) {
    //   class_test(pba->a_nodes_VCDM[i]<pba->a_nodes_VCDM[i+1], pba->error_message,
    //           "The a_array for the w-a spline must be in descending order");
    // }
    class_test(pba->w_function_index < 0 || pba->w_function_index >= function_count,
            pba->error_message,
            "w_function_index=%d is out of range. Valid range: 0 to %d",
            pba->w_function_index, function_count-1);
    // GB end  
  }
...
int background_solve(
  ...
  // AM: debugging
  // if (pba->has_lambda==_TRUE_) {
  //   printf("Struct background has lambda = true in background_solve\n");
  // }
  ...
  // printf("Background integration done: loga_ini=%e loga_final=%e\n",loga_ini,loga_final); // AM - debugging
...
int background_initial_conditions(
  ...
  // printf("Getting initial conditions at a = %e\n",a); // AM: debugging
  ...
  */
  ...
  // printf("Initial Omega_rad (at a0=1) = %e\n",Omega_rad); // AM: debugging
  ...
  /* Just checking that our initial time indeed is deep enough in the radiation
     dominated regime */
  // printf("Omega_r (at a_ini) = %e\n",pvecback[pba->index_bg_Omega_r]); // AM: debugging
...
int background_derivs(
  ...
  // AM can be commented out
  // double w_0_VCDM = pba->w_0_VCDM; // ADF: value of alpha // AM: need spline parameters here
  // double w_a_VCDM = pba->w_a_VCDM; // ADF: value of a of transition // AM: need spline parameters here
  // double rho0_VCDM = pba->Omega0_lambda * pow(pba->H0,2); // AM: remains unchanged

  // double k2_pivot_VCDM = 0.1*0.1; // ADF: an arbitrarily chosen large value for k^2 in Mpc-1 // AM: unlikely to be changed
  // double p_prime = -4./3.*H*a*rho_r;
  // double H_prime = pvecback[pba->index_bg_H_prime];
  // double rho_plus_p = pvecback[pba->index_bg_rho_plus_P_VCDM];
  // // AM: Change from here to use w(z) spline 
  // double rhoX_a = 0.3e1 * pow(a, -0.4e1 - 0.3e1 * w_0_VCDM - 0.3e1 * w_a_VCDM) * (w_a_VCDM * (a - 0.1e1) - w_0_VCDM - 0.1e1) * rho0_VCDM * exp(0.3e1 * w_a_VCDM * (a - 0.1e1));
  // double rhoX_aa = 0.9e1 * pow(a, -0.5e1 - 0.3e1 * w_0_VCDM - 0.3e1 * w_a_VCDM) * rho0_VCDM * exp(0.3e1 * w_a_VCDM * (a - 0.1e1)) * (w_a_VCDM * w_a_VCDM * a * a - 0.2e1 * w_a_VCDM * (0.1e1 + w_0_VCDM + w_a_VCDM) * a + (w_0_VCDM + w_a_VCDM + 0.4e1 / 0.3e1) * (0.1e1 + w_0_VCDM + w_a_VCDM));
  // double H_prime_prime = rhoX_aa * pow(a, 0.4e1) * H / 0.2e1 + rhoX_a * pow(a, 0.3e1) * H + 0.3e1 * a * a * H * rho_plus_p - 0.3e1 / 0.2e1 * a * p_prime;

  dy[pba->index_bi_D] = y[pba->index_bi_D_prime]/a/H; // ADF: dy -> dD/dloga // AM: unlikely to be changed
...
// AM: added functions below for spline w(z)

/**
 * Linear interpolation function for w(z) using spline nodes
 * 
 * @param a        Input: scale factor
 * @param a_nodes  Input: array of scale factor nodes (must be in ascending order)
 * @param w_nodes  Input: array of w values at the nodes
 * @param n_nodes  Input: number of nodes
 * @return         Interpolated w value at scale factor a
 */
double w_VCDM_linear_spline(double a, double *a_nodes, double *w_nodes, int n_nodes) {
    ...
    if (a <= a_nodes[n_nodes-1]) {
        return w_nodes[n_nodes-1];
    }
    
  /* Use binary search to find the correct interval.  The input
     a_nodes array is expected to be in descending order (checked
     elsewhere).  We therefore search for the smallest index `u`
     such that a_nodes[u] <= a.  The desired interpolation interval
     is then [u-1, u]. */
  l = 0;
  u = n_nodes - 1;
  while (u - l > 1) {
    i = (l + u) / 2;
    if (a_nodes[i] <= a) {
      /* mid is at or to the right of target */
      u = i;
    } else {
      /* mid is to the left of target */
      l = i;
    }
  }
  i = u;
    ...
    w_interp = w_nodes[i-1] + (w_nodes[i] - w_nodes[i-1]) * 
               (a - a_nodes[i-1]) / (a_nodes[i] - a_nodes[i-1]);
    
    return w_interp;
}

/**
 * Main w(z) spline function with derivatives
 * 
 * @param a       Input: scale factor
 * @param a_nodes Input: array of scale factor nodes
 * @param w_nodes Input: array of w values at the nodes  
 * @param n_nodes Input: number of nodes
 * @param n_deriv Input: order of derivative (0, 1, or 2)
 * @return        w or its derivative with respect to scale factor a
 */
double w_VCDM_spline(double a, double *a_nodes, double *w_nodes, int n_nodes, int n_deriv) {
    
    double w_val, slope; //, dz_da;
    int i, l, u;
    
    if (n_deriv == 0) {
        // 0th derivative: value of w at scale factor a
        return w_VCDM_linear_spline(a, a_nodes, w_nodes, n_nodes);
        
    } else if (n_deriv == 1) {
        // 1st derivative: derivative of w with respect to a
        // dw/da = (dw/dz) * (dz/da) where dz/da = -1/a^2
        
        // Handle boundary cases
        if (a >= a_nodes[0] || a <= a_nodes[n_nodes-1]) {
            return 0.0; // derivative is zero outside interpolation range
        }
        
    /* Find the interval using binary search (descending a_nodes).
       See comment in linear_spline above. */
    l = 0;
    u = n_nodes - 1;
    while (u - l > 1) {
      i = (l + u) / 2;
    ...
    /* Calculate slope dw/da in the linear segment */
    slope = (w_nodes[i] - w_nodes[i-1]) / (a_nodes[i] - a_nodes[i-1]);
        ...
        // 2nd derivative: for linear spline, second derivative is zero everywhere
        // except at the nodes where it's undefined (discontinuous)
        return 0.0;
    }
...
// GB start
/**
 * Trapezoidal integration of y = -3(1+w(a))/a over a range of scale factor values
 * 
 * @param a_min      Input: minimum scale factor
 * @param a_max      Input: maximum scale factor  
 * @param n_steps    Input: number of integration steps
 * @param func_index Input: index of function in function_table
 * @param c0         Input: first parameter for function (c0_esr)
 * @param c1         Input: second parameter for function (c1_esr)
 * @return           Integrated value using trapezoidal rule
 */
double integrate_w_function_trapezoidal(double a_start, double a_end, int n_steps,
                                        int func_index, double c0, double c1) {
    double dloga, loga, a, w_val;
    double integral = 0.0;
    double y_prev, y_curr;
    int i;

    if (n_steps < 2) {
        fprintf(stderr, "Error: n_steps must be >= 2\n");
        return 0.0;
    }

    double loga_start = log(a_start);  // = log(1) = 0
    double loga_end   = log(a_end);
    dloga = (loga_end - loga_start) / (n_steps - 1);

    eos_function w_func = function_table[func_index];

    loga  = loga_start;
    a     = exp(loga);
    w_val = w_func(a, c0, c1);
    y_prev = -3.0 * (1.0 + w_val); 

    for (i = 1; i < n_steps; i++) {
        loga  = loga_start + i * dloga;
        a     = exp(loga);
        w_val = w_func(a, c0, c1);
        y_curr = -3.0 * (1.0 + w_val);

        integral += 0.5 * (y_prev + y_curr) * dloga;
        y_prev = y_curr;
    }
...
//  * @param a_max   Input: maximum scale factor  
//  * @param n_steps Input: number of integration steps
//  * @param a_nodes Input: array of scale factor nodes for w(a) spline
//  * @param w_nodes Input: array of w values at the nodes
//  * @param n_nodes Input: number of spline nodes
//  * @return        Integrated value using trapezoidal rule
//  */
// double integrate_w_spline_trapezoidal(double a_min, double a_max, int n_steps, 
//                                      double *a_nodes, double *w_nodes, int n_nodes) {
    
//     double da, a, w_val, y_val;
//     double integral = 0.0;
//     double y_prev, y_curr;
//     int i;
    
//     if (n_steps < 2) {
//         fprintf(stderr, "Error: n_steps must be >= 2 for trapezoidal integration\n");
//         return 0.0;
//     }
    
//     da = (a_max - a_min) / (n_steps - 1);
    
//     // Calculate first point
//     a = a_min;
//     w_val = w_VCDM_spline(a, a_nodes, w_nodes, n_nodes, 0);
//     y_prev = -3.0 * (1.0 + w_val) / a;
    
//     // Trapezoidal integration
//     for (i = 1; i < n_steps; i++) {
//         a = a_min + i * da;
//         w_val = w_VCDM_spline(a, a_nodes, w_nodes, n_nodes, 0);
//         y_curr = -3.0 * (1.0 + w_val) / a;
        
//         // Add trapezoidal area
//         integral += 0.5 * (y_prev + y_curr) * da;
...
//     return integral;
// }
// // AM: end of added functions for spline w(z)
// // ----------------------------------------------------------------
// GB end
