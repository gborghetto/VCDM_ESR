...
int input_read_parameters(struct file_content * pfc,
  ...
  /** VCDM parameters (following ncdm pattern exactly) */
  // ADF: parameters of VCDM which needs to be given // AM: new parameters needed for w(a) spline
  // class_read_double("w_0_VCDM",pba->w_0_VCDM);
  // class_read_double("w_a_VCDM",pba->w_a_VCDM);

  // AM: in class we can pass in a list of doubles using class_read_list_of_doubles(_or_default) e.g. see input structure for m_ncdm
  // Read N_nodes_VCDM parameter (or use default from input_default_params)
  // class_read_int("N_nodes_VCDM", N_nodes_VCDM);
  
  // /* Complete set of VCDM parameters - always process arrays like ncdm does */
  // pba->N_nodes_VCDM = N_nodes_VCDM;
  // class_read_list_of_doubles_or_default("a_nodes_VCDM",pba->a_nodes_VCDM,1.,pba->N_nodes_VCDM);
  // class_read_list_of_doubles_or_default("w_nodes_VCDM",pba->w_nodes_VCDM,-1.,pba->N_nodes_VCDM);

  // GB start
  // Read which function to use from the table
  class_read_int("w_function_index", pba->w_function_index);
  
  // Read the two parameters for the generated function
  class_read_double("c0_esr", pba->c0_esr);
  class_read_double("c1_esr", pba->c1_esr);
...
int input_default_params(struct background *pba,
  ...
  // ADF: giving some default values for the VCDM parameters
  // pba->w_0_VCDM = -1.0;
  // pba->w_a_VCDM = 0.0;

  // AM: default values for the spline w(z), in case no input is passed by user
  // pba->N_nodes_VCDM = 2;
  // pba->a_nodes_VCDM = NULL;
  // pba->w_nodes_VCDM = NULL;

  // GB start
  pba->w_function_index = 0;  // Default to first function (CPL)
  pba->c0_esr = -1.0;              // Default w0
  pba->c1_esr = 0.0;              // Default wa
