...
import simplifier as simplifier
...
def main(runname, compl, track_memory=False, search_tmax=60, expand_tmax=1, seed=1234,dirname=None):
    ...
    if runname == 'keep_duplicates':
        ...
        basis_functions = [["x", "a"],  #  type0
                           ["sqrt_abs", "square", "exp","log_abs" ],  # type1
                           ["+", "*", "-", "/", "pow"]]

    dirname = os.path.abspath(os.path.join(os.path.dirname(
        generator.__file__), '..', 'function_library')) if dirname is None else dirname
...
if __name__ == "__main__":
    compl = 2 #int(sys.argv[1])
    runname = 'VCDM_maths' #'core_maths'
    main(runname, compl)
